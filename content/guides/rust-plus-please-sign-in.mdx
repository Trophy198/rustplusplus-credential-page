...
Steam's login page remembers the last account used in the phone's browser. If you have an alt account, or a family member signed in on the same phone, you may be signing in to the wrong one. The name at the top of the Steam login page is the account Rust+ will use. Sign out of Steam in the browser page if needed and enter the right credentials.
...
Open `https://companion-rust.facepunch.com/login` in a normal browser on your PC and sign in. If Facepunch's page itself fails or shows an error, the service is having a problem and no fix on your phone will help; check the [Rust status page](https://status.facepunch.com/) and try later.
