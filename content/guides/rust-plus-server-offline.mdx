...
- The server announces the wrong address. Servers behind NAT need `app.publicip` set to the public IP so the app connects to the right place; `app.info` shows what is being announced.
- A recent change to the game port without a matching change to `app.port`, so the app still connects to the old value.
- The port is below 10000, which Facepunch's backend does not support.
