...
If the ESC menu has no **Rust+** entry, or the entry has no **Pair with server** button, the server has it off. Nothing you do on the phone will change that. See [the pair button is missing](/guides/rust-plus-pair-button-not-showing) for how to confirm and what to tell the admin.
...
This is the single most useful check. Official Facepunch servers always have Rust+ configured correctly. Join one, press Pair.
...
Each server has a `companion.id` file in its save folder that identifies it to Facepunch. If an admin copies a server from a template, restores a backup from another machine, or runs two servers with the same file, Facepunch sees a duplicate identity and pairings become unreliable or fail outright. The fix is the admin's: give each server its own `companion.id` and never reuse one.
...
```
1. app.info in the server console shows the companion port and whether it is listening
2. that TCP port must be reachable from the internet (forwarded and allowed through the firewall)
