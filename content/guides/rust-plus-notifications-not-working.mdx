...
- No internet on the phone at that moment. Pushes are not retried forever; a phone that was offline when a raid started shows nothing later.
...
Seeing **Failed to set up notifications** or **Failed to get information from server** in the app usually means the server accepted the pairing but the follow-up registration of your device with Facepunch did not complete. Sign out of the app, sign back in, and pair again. If it repeats on one server only, that server's outbound connection to Facepunch is unreliable (step 5).
